...
class SetCommandsCommand : Command {
    ...
    override fun buildNode(node: LiteralArgumentBuilder<FabricClientCommandSource>) {
        node.then(
            ClientCommandManager.literal("use")
                .then(
                    ClientCommandManager.literal("essentials")
                        .executes {
                ...
                .then(
                    ClientCommandManager.literal("vanilla")
                        .executes {
        ...
        node.then(
            ClientCommandManager.literal("creative")
                .then(
        ...
        node.then(
            ClientCommandManager.literal("survival")
                .then(
                    ClientCommandManager.argument("command", StringArgumentType.greedyString())
                        .executes { context: CommandContext<FabricClientCommandSource?> ->
                            val command = getCommand(context)
